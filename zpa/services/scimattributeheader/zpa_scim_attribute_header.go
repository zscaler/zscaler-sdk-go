...
func GetAllAttrValues(service *services.Service, idpId, ScimAttrHeaderID, search string) ([]string, error) {
	...
	// Pass the search query to GetAllPagesGeneric
	l, _, err := common.GetAllPagesGeneric[string](service.Client, relativeURL, search)
	return l, err
}
...
func GetByName(service *services.Service, scimAttributeName, IdpId string) (*ScimAttributeHeader, *http.Response, error) {
	...
	return nil, resp, fmt.Errorf("no scim attribute named '%s' was found", scimAttributeName)
}
...
// THIS FUNCTION HAS BEEN DEPRECATED IN FAVOR OF GetAllAttrValues
