...
func (service *Service) GetByApplicationType(applicationType string, expandAll bool) ([]AppSegmentInspection, *http.Response, error) {
	if applicationType != "INSPECT" {
		return nil, nil, fmt.Errorf("invalid applicationType '%s'. Valid types are 'INSPECT'", applicationType)
	}
	...
	list, resp, err := common.GetAllPagesGenericWithCustomFilters[AppSegmentInspection](service.Client, relativeURL, filter)
	if err != nil {
