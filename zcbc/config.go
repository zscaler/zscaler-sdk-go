...
const (
	...
	// API types
	zcbcAPIVersion  = "api/v1"
	defaultProtocol = "https://"
	zcbcAPIAuthURL  = "/auth"
	loggerPrefix    = "zcbc-logger: "
)
...
// NewClientzcbc NewClient Returns a Client from credentials passed as parameters
func NewClientzcbc(username, password, apiKey, zcbcCloud, userAgent string) (*Client, error) {
	httpClient := getHTTPClient()
	var logger *log.Logger
	if loggerEnv := os.Getenv("ZSCALER_SDK_LOG"); loggerEnv == "true" {
		logger = getDefaultLogger()
	}
	url := fmt.Sprintf("https://connector.%s.net/%s", zcbcCloud, zcbcAPIVersion)
	cli := Client{
		userName:   username,
		password:   password,
		apiKey:     apiKey,
...
// MakeAuthRequestzcbc ...
func MakeAuthRequestzcbc(credentials *Credentials, url string, client *http.Client, userAgent string) (*Session, error) {
	if credentials == nil {
	...
	req, err := http.NewRequest("POST", url+zcbcAPIAuthURL, bytes.NewReader(data))
	if err != nil {
		return nil, err
	}
...
func (c *Client) refreshSession() error {
	...
	session, err := MakeAuthRequestzcbc(&credentialData, c.URL, c.HTTPClient, c.UserAgent)
	if err != nil {
		return err
	}
	c.session = session
...
func getHTTPClient() *http.Client {
	...
	retryableClient.HTTPClient.Transport = logging.NewTransport("gozscaler-zcbc", retryableClient.HTTPClient.Transport)
...
func (c *Client) Logout() error {
	_, err := c.Request(zcbcAPIAuthURL, "DELETE", nil, "application/json")
	if err != nil {
		return err
	}
	return nil
