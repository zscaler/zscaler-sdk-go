...
func TestZDXCheckRetryOnServerErrors(t *testing.T) {
	tests := []struct {
		name      string
		status    int
		body      string
...
func TestZDXCheckRetryPreservesExistingBehaviour(t *testing.T) {
...
func TestZDXErrorHandlerIsWired(t *testing.T) {
	cfg := &Configuration{}
	cfg.ZDX.Client.RateLimit.MaxRetries = 2
	cfg.ZDX.Client.RateLimit.RetryWaitMin = time.Millisecond
	cfg.ZDX.Client.RateLimit.RetryWaitMax = 2 * time.Millisecond

	httpClient := getHTTPClient(logger.GetDefaultLogger("zdx-test: "), nil, cfg)
...
func TestZDXDeterministic500NotRetriedEndToEnd(t *testing.T) {
	var attempts int32
	...
	cfg := &Configuration{}
	cfg.ZDX.Client.RateLimit.MaxRetries = 5
	cfg.ZDX.Client.RateLimit.RetryWaitMin = time.Millisecond
	cfg.ZDX.Client.RateLimit.RetryWaitMax = 2 * time.Millisecond

	httpClient := getHTTPClient(logger.GetDefaultLogger("zdx-test: "), nil, cfg)
	req, err := http.NewRequest(http.MethodGet, srv.URL+"/zdx/api/v1/x", nil)
	require.NoError(t, err)
