...
func retryErrTestResponse(status int, body string) *http.Response {
	u, _ := url.Parse("https://api.example.com/zia/api/v1/urlFilteringRules")
...
func TestOneAPICheckRetryOnServerErrors(t *testing.T) {
	tests := []struct {
		name      string
		status    int
		body      string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp := retryErrTestResponse(tt.status, tt.body)
...
func TestOneAPICheckRetryPreservesExistingBehaviour(t *testing.T) {
	t.Run("429 is retried", func(t *testing.T) {
		shouldRetry, err := checkRetry(context.Background(), retryErrTestResponse(http.StatusTooManyRequests, ""), nil)
		require.NoError(t, err)
		assert.True(t, shouldRetry)
	})

	t.Run("409 edit lock is retried", func(t *testing.T) {
		resp := retryErrTestResponse(http.StatusConflict, `{"code":"EDIT_LOCK_NOT_AVAILABLE"}`)
		shouldRetry, err := checkRetry(context.Background(), resp, nil)
		require.NoError(t, err)
		assert.True(t, shouldRetry)
	})
	...
	t.Run("412 org barrier is retried", func(t *testing.T) {
		resp := retryErrTestResponse(http.StatusPreconditionFailed, `{"code":"UNEXPECTED_ERROR","message":"Failed during enter Org barrier"}`)
		shouldRetry, err := checkRetry(context.Background(), resp, nil)
		require.NoError(t, err)
		assert.True(t, shouldRetry)
	})
	...
	t.Run("400 is not retried", func(t *testing.T) {
		shouldRetry, err := checkRetry(context.Background(), retryErrTestResponse(http.StatusBadRequest, `{"code":"INVALID"}`), nil)
		require.NoError(t, err)
	...
	t.Run("cancelled context is not retried", func(t *testing.T) {
		...
		cancel()
		shouldRetry, err := checkRetry(ctx, retryErrTestResponse(http.StatusInternalServerError, ""), nil)
		assert.False(t, shouldRetry)
		assert.Error(t, err)
	})
}

func TestOneAPIErrorHandlerIsWired(t *testing.T) {
	cfg := &Configuration{}
	cfg.Zscaler.Client.RateLimit.MaxRetries = 2
	cfg.Zscaler.Client.RateLimit.RetryWaitMin = time.Millisecond
	cfg.Zscaler.Client.RateLimit.RetryWaitMax = 2 * time.Millisecond

	httpClient := getHTTPClient(logger.GetDefaultLogger("oneapi-test: "), nil, cfg)
	...
	t.Run("preserves the response when the retry budget is exhausted", func(t *testing.T) {
		resp := retryErrTestResponse(http.StatusInternalServerError, `{"code":"UNEXPECTED_ERROR"}`)
		got, err := rt.Client.ErrorHandler(resp, nil, 3)
		require.NoError(t, err)
		require.NotNil(t, got)
		assert.Equal(t, http.StatusInternalServerError, got.StatusCode)

		// The preserved response must still convert into a structured SDK error.
		respErr, ok := errorx.AsErrorResponse(errorx.CheckErrorInResponse(got, nil))
		require.True(t, ok)
...
func TestOneAPIDeterministic500NotRetriedEndToEnd(t *testing.T) {
	var attempts int32
	...
	httpClient := getHTTPClient(logger.GetDefaultLogger("oneapi-test: "), nil, cfg)
	req, err := http.NewRequest(http.MethodPost, srv.URL+"/zia/api/v1/urlFilteringRules", bytes.NewBufferString("{}"))
	require.NoError(t, err)
