...
func TestZPACheckRetryOnServerErrors(t *testing.T) {
	tests := []struct {
		name      string
		status    int
		body      string
...
// TestZPACheckRetryPreservesExistingBehaviour guards the ZPA-specific retry
// paths that predate the #449 fix.
func TestZPACheckRetryPreservesExistingBehaviour(t *testing.T) {
	...
	t.Run("400 db.simultaneous.request is retried", func(t *testing.T) {
		resp := retryTestResponse(http.StatusBadRequest, `{"id":"db.simultaneous.request"}`)
		shouldRetry, err := checkRetry(context.Background(), resp, nil)
		require.NoError(t, err)
		assert.True(t, shouldRetry, "ZPA concurrency retry must be preserved")
	})

	t.Run("400 api.concurrent.access.error is retried", func(t *testing.T) {
		resp := retryTestResponse(http.StatusBadRequest, `{"id":"api.concurrent.access.error"}`)
		shouldRetry, err := checkRetry(context.Background(), resp, nil)
		require.NoError(t, err)
		assert.True(t, shouldRetry)
	})

	t.Run("400 unrelated id is not retried", func(t *testing.T) {
		resp := retryTestResponse(http.StatusBadRequest, `{"id":"some.other.error"}`)
		shouldRetry, err := checkRetry(context.Background(), resp, nil)
		require.NoError(t, err)
...
// TestZPAErrorHandlerIsWired asserts getHTTPClient installs an ErrorHandler that
// preserves the final API response, and still reports pure transport failures.
func TestZPAErrorHandlerIsWired(t *testing.T) {
	cfg := &Configuration{}
	cfg.ZPA.Client.RateLimit.MaxRetries = 2
	cfg.ZPA.Client.RateLimit.RetryWaitMin = time.Millisecond
	cfg.ZPA.Client.RateLimit.RetryWaitMax = 2 * time.Millisecond

	httpClient := getHTTPClient(logger.GetDefaultLogger("zpa-test: "), nil, cfg)
...
// TestZPADeterministic500NotRetriedEndToEnd exercises the real client against a
// server that always returns a deterministic 500.
func TestZPADeterministic500NotRetriedEndToEnd(t *testing.T) {
	var attempts int32
	...
	cfg := &Configuration{}
	cfg.ZPA.Client.RateLimit.MaxRetries = 5
	cfg.ZPA.Client.RateLimit.RetryWaitMin = time.Millisecond
	cfg.ZPA.Client.RateLimit.RetryWaitMax = 2 * time.Millisecond

	httpClient := getHTTPClient(logger.GetDefaultLogger("zpa-test: "), nil, cfg)
	req, err := http.NewRequest(http.MethodPost, srv.URL+"/mgmtconfig/v1/admin/x", bytes.NewBufferString("{}"))
	require.NoError(t, err)
