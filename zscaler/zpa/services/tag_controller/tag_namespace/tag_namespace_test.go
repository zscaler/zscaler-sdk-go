...
func TestNamespaceCRUD(t *testing.T) {
	...
	createdResource, _, err := Create(context.Background(), service, Namespace{
		Name:        name,
		Description: name,
		Enabled:     true,
		Origin:      "CUSTOM",
	...
	t.Run("TestUpdateStatus", func(t *testing.T) {
		...
		retrievedResource, _, err := Get(context.Background(), service, createdResource.ID)
		if err != nil {
			t.Fatalf("Error retrieving resource after status update: %v", err)
		}
		if retrievedResource.Enabled {
			t.Errorf("Expected namespace to be disabled after status update, but it is still enabled")
		}
	})
...
func TestUpdateNonExistentResource(t *testing.T) {
	...
	_, err = Update(context.Background(), service, "non_existent_id", &Namespace{})
	if err == nil {
