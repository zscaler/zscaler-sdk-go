...
// newRetryTestConfig builds the minimal configuration that makes ZCC's
// getHTTPClient construct a retryablehttp client. Unlike the other clients, ZCC
// only wires retries when BackoffConf is present and enabled.
func newRetryTestConfig(maxRetries int) *Configuration {
	cfg := &Configuration{}
	cfg.Logger = logger.GetDefaultLogger("zcc-test: ")
	cfg.ZCC.Client.RateLimit.RetryWaitMin = time.Millisecond
	cfg.ZCC.Client.RateLimit.RetryWaitMax = 2 * time.Millisecond
	cfg.ZCC.Client.RateLimit.BackoffConf = &BackoffConfig{
		Enabled:             true,
		MaxNumOfRetries:     maxRetries,
...
func TestZCCCheckRetryOnServerErrors(t *testing.T) {
	tests := []struct {
		name      string
		status    int
		body      string
...
func TestZCCCheckRetryPreservesExistingBehaviour(t *testing.T) {
...
func TestZCCErrorHandlerIsWired(t *testing.T) {
	cfg := newRetryTestConfig(2)

	httpClient := getHTTPClient(cfg.Logger, nil, cfg)
...
func TestZCCDeterministic500NotRetriedEndToEnd(t *testing.T) {
	var attempts int32
	...
	cfg := newRetryTestConfig(5)
	httpClient := getHTTPClient(cfg.Logger, nil, cfg)
	req, err := http.NewRequest(http.MethodPost, srv.URL+"/zcc/papi/public/v1/x", bytes.NewBufferString("{}"))
	require.NoError(t, err)
