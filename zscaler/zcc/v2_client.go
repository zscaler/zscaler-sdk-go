...
func getHTTPClient(l logger.Logger, rateLimiter *rl.RateLimiter, cfg *Configuration) *http.Client {
	if cfg.HTTPClient == nil {
		if cfg.ZCC.Client.RateLimit.BackoffConf != nil && cfg.ZCC.Client.RateLimit.BackoffConf.Enabled {
			...
			// When the retry budget is exhausted, retryablehttp's default behaviour
			// is to drain the body and return a bare "giving up after N attempt(s)"
			// error with a nil response, which destroys the API's own error payload.
			// Hand the final response back instead so the request layer can build a
			// structured errorx.ErrorResponse carrying the API code and message. A
			// nil response means the request never produced one (e.g. connection
			// refused), in which case the transport error is the only signal left to
			// report. See issue #449.
			retryableClient.ErrorHandler = func(resp *http.Response, err error, numTries int) (*http.Response, error) {
				// err is nil only when the retry budget ran out while CheckRetry
				// still wanted to retry. That is the case worth rescuing, and the
				// body is intact because retryablehttp breaks out of its loop
				// before draining it. Any non-nil err is a genuine failure
				// (transport error, cancelled or timed-out context surfaced by
				// CheckRetry) and must not be masked by handing back a response
				// with a nil error.
				if err == nil && resp != nil {
