...
const (
	...
	changelogProcessDocumentation = "https://github.com/zscaler/zscaler-sdk-go/blob/master/docs/changelog-process.md"
		...
		if strings.Contains(comment.GetBody(), "no changelog entry is attached to") {
			if changelogEntryPresent {
				client.Issues.EditComment(ctx, owner, repo, *comment.ID, &github.IssueComment{
					Body: zscaler.StringPtr(changelogDetectedMessage),
				})
				os.Exit(0)
			}
