...
// CreateCloudApplicationsExport sends a POST request to create a new CloudApplicationsExport
func CreateCloudApplicationsExport(service *services.Service, entity string, appExport *CloudApplicationsExport) (*CloudApplicationsExport, *http.Response, error) {
	...
	// Create a map to include the entity and the CloudApplicationsExport
	payload := map[string]interface{}{
		"entity":  entity,
		"payload": appExport,
	}
	resp, err := service.Client.Create(appExportEndpoint, payload)
	if err != nil {
		return nil, nil, err
	}
	createdExport, ok := resp.(*CloudApplicationsExport)
	if !ok {
		return nil, nil, errors.New("object returned from API was not a CloudApplicationsExport pointer")
	}
